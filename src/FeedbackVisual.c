...
void controle_display(){
    ...
    if(menu){
        ...
        if(gpio_get(ACI_BOMBA)){
            ssd1306_draw_string(&ssd, "On", 98, 48); // Desenha uma string
        }else{
            ssd1306_draw_string(&ssd, "Off", 98, 48); // Desenha uma string
        }

    }else{
        ssd1306_rect(&ssd, 2, 115, 10, 59, true, false); // Borda
        float nivel = 60;
        float y = (61 - (nivel*0.59));
        ssd1306_rect(&ssd, y, 115, 10, (61 - y), true, true); // Preenchimento

        float y_max = (61 - (altura_maxima*0.59));
        float y_min = (61 - (altura_minima*0.59));
        ssd1306_line(&ssd, 114, y_max, 124, y_max, true); // Desenha uma linha horizontal (altura máxima)
        ssd1306_line(&ssd, 114, y_min, 124, y_min, true); // Desenha uma linha horizontal (altura mínima)
